...
public class SearchByCodeViewController implements Initializable {
	//Attribute
	private Main main;
	...
	public SearchByCodeViewController(){
	...
	void refreshData(ActionEvent e) {
		try {
			...
			if(main.thereArePeopleInSystem()== true) {
				...
				ObservableList<Person> aux = main.refreshData(input.toUpperCase(),3);
				
				if(aux.size()==0) {
					tableData.setItems(aux);
					Alert alert = new Alert(AlertType.CONFIRMATION);
					alert.setTitle("Successful process");
					alert.setHeaderText("ID not found in database");
					alert.showAndWait();
				}
				else {
					tableData.setItems(aux);
